...
impl<'a> TickaContext<'a> {
    pub fn new(unit_locations: &'a mut MobileEntityField2D<Unit>, unit_locations_new: Option<MobileEntityField2D<Unit>>, unit_move_sender: Option<Sender<UnitMoveInstance>>,  fields: &'a mut Vec<Field2D<f32>>) -> Self {

        //let unit_locations: MobileEntityField2D<Unit> = MobileEntityField2D::new(width, height)
        TickaContext { unit_move_sender, unit_locations, unit_locations_new, fields }
    }
